...
use crate::crypt::{
    AES_NONCE_LENGTH, EncryptedDataArr, IV_LENGTH, decrypt_region, decrypt_region_dyn, encrypt_region, generate_user_key
};
use crate::vault::entry::{DirectoryEntry, EncryptedEntry, Entry, EntryResult, PasswordEntry, SecretFileEntry, VaultEntry};
use crate::vault::error::{DeleteEntryError, EntryRenameError, EntryType, NewEntryError, Operation, ReadVaultFileError, RenameEntryError, RetrieveSecretError, VaultChangeEntryError, VaultChangeError, VaultError};
use crate::vault::utils::{BlockRange, BlockSet, VaultPath, read_dyn_field, read_field};
...
impl<'a> VaultManager<'a> {
    ...
    pub fn retrieve_secret_entry(&self, entry_path: String) -> Result<EntryResult, RetrieveSecretError> {
    ...
    pub fn rename(&mut self, entry_path: String, new_name: String) -> Result<(), RenameEntryError> {
        let path = VaultPath::new(entry_path.clone()).map_err(|e| RenameEntryError::InvalidVaultPath(e))?;
        self.root_entry.rename_entry(path.parts().into(), &path, new_name).map_err(|e| EntryRenameError::VaultError(e));
        Ok(())
    }
    ...
    pub fn change_password(
        ...
        let path = VaultPath::new(entry_path.clone()).map_err(|e| VaultChangeEntryError::InvalidVaultPath(e))?;
        let entry = self.root_entry.get_entry_mut(path.parts().into(), &path)?;
        
         match entry {
            VaultEntry::Password(pwd) => pwd.change_secret(&mut self.context, &self.vault_key, password).map_err(|e| VaultChangeEntryError::VaultChangeError(e)),
                VaultEntry::Directory(_) => Err(VaultChangeEntryError::InvalidOperation(Operation::ChangePassword, EntryType::Directory)),
                VaultEntry::Secret(_) => Err(VaultChangeEntryError::InvalidOperation(Operation::ChangePassword, EntryType::Secret))
        }
    }
    ...
    pub fn change_secret(
        ...
        match entry {
            VaultEntry::Password(_) => Err(VaultChangeEntryError::InvalidOperation(Operation::ChangeSecret, EntryType::Password)),
                VaultEntry::Directory(_) => Err(VaultChangeEntryError::InvalidOperation(Operation::ChangeSecret, EntryType::Directory)),
                VaultEntry::Secret(sec) => sec.change_secret(&mut self.context, &self.vault_key, secret_file_path).map_err(|e| VaultChangeEntryError::VaultChangeError(e))
        }
    }

    pub fn delete_entry(&mut self, entry_path: String) -> Result<(), DeleteEntryError> {
        let path = VaultPath::new(entry_path.clone()).map_err(|e| DeleteEntryError::InvalidVaultPath(e))?;
        self.root_entry
            .delete_entry(path.parts().into(), &path, &mut self.context).map_err(|e| DeleteEntryError::VaultError(e))
    }

    pub fn new_directory(&mut self, dir_path: String, dir_name: String) -> Result<(), NewEntryError> {
        let entry = VaultEntry::Directory(DirectoryEntry::new(dir_name).map_err(|e| NewEntryError::NameError(e))?);
        let path = VaultPath::new(dir_path).map_err(|e| NewEntryError::InvalidVaultPath(e))?;
        if let Some(parent_path) = path.parent() {
            self.root_entry.new_entry(path.parts().into(), &parent_path, entry).map_err(|e| NewEntryError::VaultError(e));
        }
        Ok(())
    }

    pub fn new_secret(&mut self, entry_path: String, file_path: String) -> Result<(), NewEntryError> {
        let path = VaultPath::new(entry_path).map_err(|e| NewEntryError::InvalidVaultPath(e))?;
        if path.name() {
            // if a name does not exist -> Error
            // TODO Adjust errors and path based handling of these functions
        }  
        let secret = SecretFileEntry::new(path.name().to_string(), file_path, &mut self.context, &self.vault_key).map_err(|e| NewEntryError::VaultChangeError(e))?;
        let parent_path = path.parent().unwrap();
        self.root_entry.new_entry(path.parts().into(), &parent_path, VaultEntry::Secret(secret)).map_err(|e| NewEntryError::VaultError(e))
    }
}
...
impl VaultChangeContext {
    pub fn new(root_entry: &DirectoryEntry) -> Self {
        VaultChangeContext { changes: Vec::new(), empty_blocks: root_entry.occupied_datablocks() }
    }
}
...
fn read_header(reader: &mut BufReader<File>) -> Result<HeaderInfo, ReadVaultFileError> {
    ...
    let signature = u64::from_ne_bytes(
        read_field::<VAULT_SIGNATURE_LENGTH>(reader)
            .map_err(|e| ReadVaultFileError::ReadEntryError(e, Some(offset)))?,
    );
    ...
    let version = read_field::<VAULT_VERSION_LENGTH>(reader)
        .map_err(|e| ReadVaultFileError::ReadEntryError(e, Some(offset)))?[0];
    ...
    let vaultname_raw = read_field::<VAULTNAME_LENGTH>(reader)
        .map_err(|e| ReadVaultFileError::ReadEntryError(e, Some(offset)))?;
    let vaultname = String::from_utf8(vaultname_raw.to_vec()).map_err(|e| {
        ReadVaultFileError::ReadEntryError(ReadFieldError::ReadUtf8Error(e), Some(offset))
    })?;
...
fn read_vault_table(
    ...
    let vault_table = read_dyn_field(reader, header.vault_table_size as usize * VAULTENTRY_LENGTH)
        .map_err(|e| ReadVaultFileError::ReadEntryError(e, None))?;
    ...
    loop {
        ...
        if remaining_size == 0 {
            ...
            let dir = res_dir.unwrap().1;
            {
                ...
                cur_dir.unwrap().1.add_vaultentry(VaultEntry::Directory(dir));
            }
        }
    }
}
    ...
    match entry_data[0] {
        DIRENTRY_TYPE => {
            ...
            let name = String::from_utf8(entry_data[offset..offset + VAULTENTRY_LENGTH].to_vec())
                .map_err(|e| {
                ReadVaultFileError::ReadEntryError(ReadFieldError::ReadUtf8Error(e), Some(offset as u64))
            })?;
